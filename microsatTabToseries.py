...
for line in file:
    if info == "skip":
        # adds new column "series"
        sortie = line[0:len(line)-1] + "\t" + "series"
        fileout.write(sortie)
        fileout.write('\n')
        info = "go"
    else:
        # splits on \t and takes out sequence (last element)
        element = line.split()
        seqsat = element[len(element) - 1]
        ...
        if found == 1:
            ...
            found = 0
            cpt = -1
            # find position where first motif sequence appears
            while found == 0 and cpt < len(seqsat)-lenM:
                cpt = cpt + 1
                seq = seqsat[cpt:cpt + lenM]
                if seq == motif:
                    found = 1
            endseq1 = cpt - 1
            # seq1 is sequence up to first appearance of motif
            seq1 = seqsat[0:(endseq1+1)]
            currentList[0] = seq1

            # find position where motif sequence repetition ends
            found = 0
            cpt = len(seqsat) - lenM * 2 + 1
            while found == 0 and cpt > -1:
                cpt = cpt - 1
            ...
            startseq2 = cpt + lenM * 2
            # seq2 is sequence from last motif to the end
            seq2 = seqsat[startseq2:len(seqsat)]
            currentList[9] = seq2
            ...
            debm = 0
            endm = 0
            cpt = endseq1 + 1  # start from beginning of the sequence where motif appears
            nbmotif = 0

            # slider of width lenM runs right-wards and counts motif occurrences
            while cpt < (startseq2-lenM):
                currentseq = seqsat[cpt:cpt + lenM]
                if currentseq == motif:
                    cpt = cpt + lenM
                else:
                    # this is triggered in case the motif is modified
                    debm = cpt
                    while currentseq != motif:
                        # counter counts how many positions (each position is len(motif)) are modified
                        cpt = cpt + 1
                        currentseq = seqsat[cpt:cpt + lenM]
                    endm = cpt - 1
                    motifm = seqsat[debm:endm + 1]
            ...
            # search into family data
            if first == 1:
                seq1_all.append(currentList[0])  # added in line ~101, start of sequence
                motifm1_all.append(currentList[1])  # lines for fm1-fm4 added in line(s) ~140
                posmotifm1_all.append(currentList[2])
                motifm2_all.append(currentList[3])
                posmotifm2_all.append(currentList[4])
                motifm3_all.append(currentList[5])
                ...
                motifm4_all.append(currentList[7])
                posmotifm4_all.append(currentList[8])
                seq2_all.append(currentList[9])  # added in line ~115, end of sequence
                sortie = line[0:len(line) - 1] + "\t" + str(nextType)
                fileout.write(sortie)
                fileout.write('\n')
                nextType = nextType + 1
                ...
                found = 0
                while found == 0 and cpt < len(seq1_all):
                    if seq1_all[cpt] == currentList[0] and \
                                    motifm1_all[cpt] == currentList[1] and \
                                    posmotifm1_all[cpt] == currentList[2] and \
                                    motifm2_all[cpt] == currentList[3] and \
                                    posmotifm2_all[cpt] == currentList[4] and \
                                    motifm3_all[cpt] == currentList[5] and \
                                    posmotifm3_all[cpt] == currentList[6] and \
                                    motifm4_all[cpt] == currentList[7] and \
                                    posmotifm4_all[cpt] == currentList[8] and \
                                    seq2_all[cpt] == currentList[9]:
                        seqType = cpt + 1
                        found = 1
                        sortie = line[0:len(line) - 1] + "\t" + str(seqType)
                        fileout.write(sortie)
            ...
            for i in range(0, 10):
                currentList.append("")
>>>>>>> 41a3507b4fa416cfa201cd43bcdcd53e05fa9299
